...
- **yt-dlp 依赖 JS 运行时**：`Dockerfile` 必须安装 `nodejs`（`ytdlp_handler._get_js_runtimes()` 检测 `node`/`deno`/`bun` 并注入 `js_runtimes` + `remote_components: ["ejs:github"]` 以解决 YouTube `n` 挑战，缺失会导致 `HTTP 403 Forbidden`，见 `doc/DEVELOPMENT.md`）。
